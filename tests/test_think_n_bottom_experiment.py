...
class ThinkNBottomExperimentTest(unittest.TestCase):
    ...
    def test_run_experiment_uses_lower_prefix_dtr_and_saves_rep_metrics(self, _tokenizer_mock):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["docs"][0]["metrics"]["selected_word_seq_rep_2"], 0.0)
            self.assertGreater(payload["docs"][0]["metrics"]["full_word_seq_rep_2"], 0.0)
            self.assertIn("bottom_maj@2: 0.000000", rendered)
            self.assertIn("selected_token_seq_rep_2", rendered)
            self.assertIn("full_word_seq_rep_4", rendered)
    ...
    def test_run_experiment_sets_rep_n_zero_for_short_sequences(self, _tokenizer_mock):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload = json.loads(summary_json.read_text(encoding="utf-8"))
            self.assertEqual(payload["docs"][0]["metrics"]["selected_token_seq_rep_2"], 0.0)
            self.assertEqual(payload["docs"][0]["metrics"]["selected_word_seq_rep_4"], 0.0)
    ...
    def test_run_experiment_computes_repetition_after_reasoning_tag_removal(self, _tokenizer_mock):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            rendered = summary_txt.read_text(encoding="utf-8")
            self.assertEqual(payload["docs"][0]["selected_repeat_indices"], [2, 3])
            self.assertEqual(payload["docs"][0]["metrics"]["full_word_seq_rep_2"], 0.0)
            match = re.search(
                r"^full_word_seq_rep_2:\s*([0-9]+(?:\.[0-9]+)?)$",
                rendered,
                re.MULTILINE,
            )
