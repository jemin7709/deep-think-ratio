...
class AggregateDtrLengthScatterTest(unittest.TestCase):
    def test_discover_summary_paths_recurses_and_skips_aggregate_dir(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            run_summary = (
                ...
                / "stamp"
                / "dtr_length_scatter"
                / "dtr_length_scatter.full.json"
            )
            run_summary.parent.mkdir(parents=True)
            write_summary(run_summary, run_dir="/tmp/run-0")

            skipped_summary = (
                root / DEFAULT_AGGREGATE_DIR_NAME / "dtr_length_scatter.full.json"
            )
            skipped_summary.parent.mkdir()
            write_summary(skipped_summary, run_dir="/tmp/aggregate")

            ignored_reasoning = (
                root / "other" / "dtr_length_scatter" / "dtr_length_scatter.reasoning.json"
            )
            ignored_reasoning.parent.mkdir(parents=True)
            write_summary(
                ignored_reasoning,
                run_dir="/tmp/reasoning",
                length_mode=REASONING_LENGTH_MODE,
            )

            paths = discover_summary_paths(
                root,
                DEFAULT_AGGREGATE_DIR_NAME,
                DEFAULT_LENGTH_MODE,
            )
    ...
    def test_load_source_summaries_rejects_mixed_length_modes(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            path_a = root / "run_a.json"
            path_b = root / "run_b.json"
            write_summary(path_a, run_dir="/tmp/run-a", length_mode=DEFAULT_LENGTH_MODE)
            write_summary(
                path_b,
                run_dir="/tmp/run-b",
                length_mode=REASONING_LENGTH_MODE,
            ...
            with self.assertRaisesRegex(ValueError, "length_mode"):
                load_source_summaries([path_a, path_b], length_mode=DEFAULT_LENGTH_MODE)
    ...
    def test_write_aggregated_json_emits_expected_metadata(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(payload["length_mode"], DEFAULT_LENGTH_MODE)
            self.assertEqual(payload["source_count"], 1)
            self.assertEqual(payload["run_dirs"], ["/tmp/run"])
            self.assertEqual(payload["num_sequences"], 2)
            self.assertEqual(output_path.name, "dtr_length_scatter_all_runs.full.json")
            self.assertEqual(
                payload["plot_path"], str(aggregate_dir / OUTPUT_PLOT_FILENAME)
            )
            self.assertEqual(payload["correct_count"], 1)
            self.assertEqual(payload["incorrect_count"], 1)
            self.assertAlmostEqual(payload["correct_length_mean"], 10)
            self.assertAlmostEqual(payload["incorrect_length_mean"], 20)
            self.assertIn("is_correct", payload["points"][0])
            self.assertEqual(
                [point["is_correct"] for point in payload["points"]], [True, False]
            )

    def test_write_aggregated_json_defaults_empty_accuracy_groups_to_null(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            aggregate_dir = root / DEFAULT_AGGREGATE_DIR_NAME
            aggregate_dir.mkdir()
            ...
            self.assertEqual(payload["incorrect_count"], 0)
            self.assertAlmostEqual(payload["correct_length_mean"], 15)
            self.assertIsNone(payload["incorrect_length_mean"])
